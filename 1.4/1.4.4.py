"""ATUALIZAÇÕES: - reformulação do código para sub-funções"""
#Definições base-----------
import random
jogadores = {}
buff_suporte = 0
...
def rolagem_dados(k, v: int, a: int) -> tuple[int, int, int]:
    acerto = 1
    ...
    def calculo_armas(dado: int, acerto: int) -> tuple[int, int]:
        dano = 0
        match v:
            case 1: #Adiciona +2 de dano ao rolamento
                dano = dado + 2
            case 2: #Caso a rolagem seja alta, dá 7 de dano. Caso contrário, dá 1 de dano
                dano = 1 if dado <= 3 else 7
            case 3: #A cada 2 turnos, dá o dobro de dano
                dano = dado * 2 if a % 2 == 1 else dado
            case 4: #diminui -1 ao seu próprio dano, mas aumenta +1 aos seus aliados
                dano = dado - 2
            case 5: #Tem maior chance de acertos críticos
                dano = dado
                acerto = 6
            case _:
                print('erro bizarro')
...
def inicio() -> tuple[bool, int]:
    def escolha_dados() -> bool:
        global dado_virt
        while True:
...
#Início----------------
print('-------------RPG Game-------------')
def boss_battle(inicio: tuple[bool, int]) -> None:
    try:
